      ...
      <div className="max-w-5xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start lg:items-center">
          ...
          <div>
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight leading-tight mb-5" style={{ color: 'var(--tx1)' }}>
              {t.contact.titleMain}<br />
              <span className="grad-text italic">{t.contact.titleAccent}</span>
            </h2>
            <p className="leading-relaxed mb-7 text-base sm:text-lg" style={{ color: 'var(--tx2)' }}>
              {t.contact.sub}
            </p>
              ...
              {[
                {
                  ...
                  icon: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>,
                ...
                <a key={row.href} href={row.href} target={row.external ? '_blank' : undefined} rel={row.external ? 'noopener noreferrer' : undefined}
                  className="flex items-center gap-4 p-4 rounded-2xl transition-all group"
                  style={{ background: 'var(--glass-bg)', border: '1px solid var(--glass-border)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)' }}
                  ...
                  <div>
                    <p className="text-xs mb-0.5" style={{ color: 'var(--tx3)' }}>{row.label}</p>
                    <p className="font-semibold text-sm transition-colors" style={{ color: 'var(--tx1)' }}>{row.value}</p>
                  </div>
                </a>
              ))}
            </div>
          ...
          {result?.success ? (
            ...
            <form action={handleSubmit} className="glass-strong p-6 sm:p-8 space-y-4">
